# Lab_BNK48
import pygame as pg
import math

# TODO 1 : กำหนด width : 1000 , height : 600 และ FPS : 60
width = 1000
height = 600
FPS = 60

# TODO 2 : กำหนดค่าสีดังนี้ pink : (197,142,195) , white : (255,255,255)
pink = (197, 142, 195)
white = (255, 255, 255)
color_list_iterator = 0
color_list = [
    (197, 142, 195),
    (0, 255, 0),
    (255, 255, 0),
    (0, 255, 255),
    (255, 0, 255)
]
Heavy_Collision_font_size = 150

# TODO 3 : กำหนดความเร็วให้กับ member แต่ละคน [ 3 member ]
# ball1_speed = [2,2]
# [-3,4]
# [3,-2]
ball_speed = list()
ball_speed.append([2, 2])
ball_speed.append([-3, 4])
ball_speed.append([3, -2])
ball_speed.append([1, -3])
ball_speed.append([5, -4])

# TODO 4 : initialize pygame variable and create clock
pg.init()
clock = pg.time.Clock()
running = True

# TODO 5 : create screen [pygame.display.set_mode]
# and set caption [pygame.display.set_caption] => "BNK_BALL (Heavy Collision)"
screen = pg.display.set_mode((width, height))
pg.display.set_caption("BNK_BALL (Heavy Collision)")

# TODO 6
# Load sound [change your sound filepath according to your computer]
pg.mixer.init()
pg.mixer.music.load("source/sound.mp3")
pg.mixer.music.play(-1, 0.0)

# ใช้คำสั่ง soundeffect.play() เพื่อเล่นเสียง effect ตอนลูกบอลชนกัน
soundeffect = pg.mixer.Sound("source/effect.wav")

# Choose 3 members from BNK48 and create pygame object from  get_rect
# [ load , resize , get_rect ]

# Member 1 [size : (150 , 150) , center : (500 , 250) ]
# ball1_img = pg.image.load("source/BNK48/Wee_cc.png").convert_alpha()
# ball1_img = pg.transform.scale(ball1_img, (150, 150))
# ball1_rect = ball1_img.get_rect(center=(500,250))

# TODO 7 : create object with attribute in each comment
# Member 2 [size : (100 , 100) , center : (250 , 120)]
# Member 3 [size : (120 , 120) , center : (800 , 400)]

ball_img = list()
ball_rect = list()
datalist = [
    ["Hoop_cc.png", (150, 150), (500, 250)],
    ["Ja_cc.png", (100, 100), (250, 120)],
    ["Mean_cc.png", (120, 120), (800, 400)],
    ["Fame_cc.png", (80, 80), (900, 500)],
    ["Jib_cc.png", (50, 50), (300, 350)]
]
radiusList = list()
for i in datalist:
    radiusList.append(i[1][0]/2)

for data in datalist:
    ball_img.append(pg.image.load("source/BNK48/"+data[0]).convert_alpha())
    ball_img[-1] = pg.transform.scale(ball_img[-1], data[1])
    ball_rect.append(ball_img[-1].get_rect(center=data[2]))

while running:
    # TODO 8 : set ให้ตัวเกมส์แสดงผลด้วยความเร็วที่เหมาะสม [clock.tick(...)]
    clock.tick(FPS)

    for event in pg.event.get():
        if event.type == pg.QUIT:
            running = False
            pg.quit()

    if running:
        # TODO 9 :ใส่สี background สีชมพู (screen.fill(...))
        screen.fill(color_list[color_list_iterator])

        # TODO 10 : ให้ member ทั้ง 3 คนเคลื่อนที่ตามทิศทางและความเร็วเป็นไปตาม speed ของแต่ละคน
        # ball1_rect = ball1_rect.move(ball1_speed)
        for i in range(0, len(datalist)):
            ball_rect[i] = ball_rect[i].move(ball_speed[i])

        # TODO 11 : วาด text คำว่า "Heavy Collision" [size : 150 , center :(width/2 , height/3), สีขาว]
        font_name = pg.font.match_font('arial')  # กำหนดชื่อ Font
        font = pg.font.Font(font_name, Heavy_Collision_font_size)  # กำหนดขนาด font
        text_surface = font.render("Heavy Collision", True, white)
        text_rect = text_surface.get_rect()
        text_rect.center = (int(width/2), int(height/3))
        screen.blit(text_surface, text_rect)

        # TODO 12 : วาด text รหัสนิสิต ลงไป ข้างใต้คำว่า "Heavy Collision" [size : 100 ,center :(width/2 , height/1.5), สีขาว]
        # [ขนาดและตำแหน่งสามารถปรับได้ตามความเหมาะสม]
        font = pg.font.Font(font_name, 100)
        text_surface = font.render("6332028421", True, white)
        text_rect = text_surface.get_rect()
        text_rect.center = (int(width/2), int(height/1.5))
        screen.blit(text_surface, text_rect)

        # TODO 13 : เขียนเงื่อนไขไม่ให้ตกกรอบทุกด้านให้กับ member ทั้ง 3 คน
        # if ball1_rect.left < 0 or ball1_rect.right > width:
        #     ball1_speed[0] = -ball1_speed[0]
        # if ball1_rect.top < 0 or ball1_rect.bottom > height:
        #     ball1_speed[1] = -ball1_speed[1]
        for i in range(0, len(datalist)):
            if ball_rect[i].left < 0 or ball_rect[i].right > width:
                ball_speed[i][0] *= -1
            if ball_rect[i].top < 0 or ball_rect[i].bottom > height:
                ball_speed[i][1] *= -1

        # Special point ทำให้ลูกบอลชนกันแล้วเด้งในทิศตรงกันข้าม
        for i in range(0, len(datalist)):
            for j in range(i+1, len(datalist)):
                iCenter = [(ball_rect[i].right+ball_rect[i].left)/2, (ball_rect[i].top+ball_rect[i].bottom)/2]
                jCenter = [(ball_rect[j].right+ball_rect[j].left)/2, (ball_rect[j].top+ball_rect[j].bottom)/2]
                if (math.sqrt(((iCenter[0]-jCenter[0])**2)+((iCenter[1]-jCenter[1])**2)) <= radiusList[i]+radiusList[j]):
                    soundeffect.play()
                    color_list_iterator += 1
                    color_list_iterator %= len(color_list)
                    Heavy_Collision_font_size += 5
                    if (Heavy_Collision_font_size > 200):
                        Heavy_Collision_font_size = 150
                    ball_speed[i][0] *= -1
                    ball_speed[i][1] *= -1
                    ball_speed[j][0] *= -1
                    ball_speed[j][1] *= -1
                    ################################################

                    # TODO 14 : เอาภาพของ member แต่ละคนใส่ลงใน object ของตนเอง
                    # screen.blit(ball1_img, ball1_rect)
        for i in range(0, len(datalist)):
            screen.blit(ball_img[i], ball_rect[i])

        ##########################################################

        pg.display.flip()
